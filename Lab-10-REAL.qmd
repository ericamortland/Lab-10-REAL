...
cities$dist_to_national_border <- st_distance(cities, usa_outline) |>
  set_units("km") |>
  drop_units()
...
```{r}
mexico_border <- countries |> filter(NAME == "Mexico") |> 
  st_union() |> 
  st_cast("MULTILINESTRING")
...
```{r}
canada_border <- countries |> filter(NAME == "Canada") |> 
  st_union() |> 
  st_cast("MULTILINESTRING")
...
```{r}
# Define the total border (Canada + Mexico) to compare distance
border <- st_union(st_geometry(canada_border), st_geometry(mexico_border)) |> 
  st_cast("MULTILINESTRING")

# Add combined border distance
cities$border_km <- set_units(st_distance(cities, border), "km") |> drop_units()

# Cities within 160 km (~100 miles) of international borders
...
ggplot() +
  ...
  labs(
    title = "U.S. Cities Within 100 Miles of Border",
